...
public class HmacSigner {
    ...
    public static String generateSecret() throws HmacException {
        try {
            return Base64.getEncoder().encodeToString(generateToken().getBytes("UTF-8"));
    ...
    public static String signJWT(String secret, String jwtID, Integer ttl,String iss, Map<String,String> claims) throws JOSEException {
        ...
        builder
                .jwtID(jwtID)
                .expirationTime(Date.from(LocalDateTime.now().plusMinutes(ttl).toLocalDate().atStartOfDay(ZoneId.systemDefault()).toInstant()))
    ...
    public static Boolean isJwtExpired(String jwtString) throws ParseException {
        ...
        if(jwt.getJWTClaimsSet() != null && jwt.getJWTClaimsSet().getExpirationTime() != null) {
        	LocalDateTime expirationDate = LocalDateTime.ofInstant(jwt.getJWTClaimsSet().getExpirationTime().toInstant(), ZoneId.systemDefault());
